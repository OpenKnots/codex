...
async fn run_guardian_review(
    ...
    let denied_action = guardian_action_value(&request);
...
fn guardian_action_value(action: &GuardianApprovalRequest) -> Value {
    match action {
...
fn format_guardian_action_pretty(action: &GuardianApprovalRequest) -> String {
    let mut value = guardian_action_value(action);
